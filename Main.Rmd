...
# model <- randomForest(taste ~ . - quality, data = EstimationData)
```
...
confusionMatrix(reference = TestData[, "taste"], data = pred)
...
The model is strongly **biased towards** the **faulty** classification, which is what it was desinged to do. The model is pretty good at predicting the **good** wines. Although the model might seem to be terrible at dealing with the **average** wines (usually splitting them 50% average, 30% faulty, 20% good) we do not care about upward misclassifications. Furthermore, with such an abundance of average wines losing out on 30% of them is not so terrible.
...
Finally, by tweaking the input parameters of the random forest algorithm, we were able to **reduce false negatives** on **faulty** wines from about to about **80-90%** down to **10-20%**.
