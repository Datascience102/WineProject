...
All the data is numeric - that's fantastic! So, just for fun, let's zoom out really far and have a look at histograms of all the attributes:
...
The main objective of our excercise is to help identify aweful wine based on its chemical attributes. These faulty wines have been defined in our dataset based on their **quality**:

```{r echo=FALSE, message=FALSE, prompt=FALSE}
barplot(table(ProjectData[,"quality"]), main = "Distribution of wine quality", xlab = "Quality", ylab = "Number of wines")
```

Wines with a quality below 5 are **faulty** - we **do not** want to **drink** these - in fact we do not want to touch them with a ten foot pole.
...
Let's start by looking at the results - how many did we get right, how many did we get wrong and what are the missclassification errors like?
...
The model is strongly **biased towards** the **faulty classification**, which is what it was desinged to do. The model is pretty good at predicting the good wines. Although the model might seem to be terrible at dealing with the average wines (usually splitting them 45% average, 35% faulty, 20% good) we do not care about upward misclassifications. Furthermore, with such an abundance of average wines losing out on 35% of them is not so terrible.
...
Or more intuitively, if you **randomly picked 1000** vino verde white wines you could expect to get **`r round(sum(TestData[,"taste"]=="faulty")/nrow(TestData)*1000,0)` faulty** wines, if you pre-selected based on a **basic random forest model** you could expect to get **`r round(sum(base.confusion.matrix$table[c(1,3),2])/sum(base.confusion.matrix$table)*1000,0)` faulty** wines, and if you pre-selected based on the **tuned random forest** model you could expect to get **`r round(sum(confusion.matrix$table[c(1,3),2])/sum(confusion.matrix$table)*1000,0)` faulty** wines.
